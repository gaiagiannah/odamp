...
pub(crate) struct AppState {
    ...
    pub(crate) sanctions: std::sync::Arc<tokio::sync::RwLock<SanctionsList>>,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    // Attempt a real OFAC fetch at startup. If it fails (network
    // issue, endpoint moved, parsing turns up nothing) we fall back
    // to the placeholder rather than refusing to start — consistent
    // with the whitepaper's "graceful degradation" principle
    // (Section 9.2) — but this is loudly logged as a warning because
    // running on the placeholder list means sanctions screening is
    // NOT actually protecting anything beyond the one test address.
    let http_client = reqwest::Client::new();
    let sanctions = match odamp_compliance::ofac::fetch_and_build(
        &http_client,
        odamp_compliance::ofac::DEFAULT_OFAC_URL,
    )
    .await
    {
        Ok(list) if !list.is_empty() => {
            tracing::info!(
                "loaded {} candidate sanctioned addresses from live OFAC fetch",
                list.len()
            );
            list
        }
        Ok(_) => {
            tracing::warn!(
                "OFAC fetch succeeded but extracted zero addresses — the heuristic \
                 parser in core/compliance/src/ofac.rs likely needs fixing against \
                 the real document shape. Falling back to placeholder list."
            );
            placeholder_sanctions_list()
        }
        Err(e) => {
            tracing::warn!(
                "OFAC fetch failed ({e}) — falling back to placeholder list. \
                 Sanctions screening is NOT live until this is fixed."
            );
            placeholder_sanctions_list()
        }
    };
    ...
    let state = AppState {
        pool,
        sanctions: std::sync::Arc::new(tokio::sync::RwLock::new(sanctions)),
    };
    ...
    let app = Router::new()
        ...
        .route("/api/v1/portfolio/:user_id/sync", post(sync::sync_positions))
        .route("/api/v1/portfolio/:user_id", get(get_portfolio))
        .route("/api/v1/compliance/screen", post(screen_address))
        .route("/api/v1/compliance/refresh", post(refresh_sanctions))
        .with_state(state);
...
fn placeholder_sanctions_list() -> SanctionsList {
...
/// Manually trigger a re-fetch of the OFAC list without restarting
/// the process. A real deployment would call this on a schedule
/// (e.g. every few hours via a cron job or a background tokio task)
/// rather than requiring a manual hit — that scheduling is not
/// wired up yet in Phase 3.
async fn refresh_sanctions(
    State(state): State<AppState>,
) -> Result<Json<serde_json::Value>, (axum::http::StatusCode, String)> {
    let http_client = reqwest::Client::new();
    let fresh = odamp_compliance::ofac::fetch_and_build(
        &http_client,
        odamp_compliance::ofac::DEFAULT_OFAC_URL,
    )
    .await
    ...
    let count = fresh.len();
    let source = fresh.source_label.clone();
    *state.sanctions.write().await = fresh;
...
async fn screen_address(
    ...
    let (blocked, results) = {
        let sanctions = state.sanctions.read().await;
        sanctions.screen_transaction(&req.from_address, &req.to_address)
    };
...
pub(crate) fn internal_error(e: anyhow::Error) -> (axum::http::StatusCode, String) {
